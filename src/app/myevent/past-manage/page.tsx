...
import MyEventVenues from '@/components/units/MyEvent/MyEventVenues';
...
export default function PastEventManagement() {
  // --- Recoil State ---
  ...
  const fetchMyEvents = useCallback(async () => {
    ...
    try {
      ...
      const response = await getMyEventPast(accessToken, 1, 10, regionParam);
      const eventData = response.data?.eventResponseDTOS || [];

      setPastEvents(Array.isArray(eventData) ? eventData : []);
      setPastHasMore(Array.isArray(eventData) ? eventData.length === 10 : false);
  ...
  }, [accessToken, setLikedClubs, setHeartbeatNums, selectedRegions]);

  useEffect(() => {
    fetchMyEvents();
  }, [fetchMyEvents]);

  // ❗ 무한스크롤 로드 더 많은 데이터 (past만)
  const loadMore = useCallback(async () => {
    if (!pastHasMore || isLoadingMore || !accessToken) return;

    setIsLoadingMore(true);
    try {
      const nextPage = pastPage + 1;
      const regionParam = selectedRegions.length > 0 ? selectedRegions[0] : undefined;
      const response = await getMyEventPast(accessToken, nextPage, 10, regionParam);
      const eventData = response.data?.eventResponseDTOS || [];
  ...
  }, [pastHasMore, pastPage, isLoadingMore, accessToken, setLikedClubs, setHeartbeatNums, selectedRegions]);

  // IntersectionObserver를 사용한 무한스크롤
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && pastHasMore && !isLoadingMore) {
          loadMore();
        }
      },
      { threshold: 0.1 },
  ...
  }, [loadMore, pastHasMore, isLoadingMore]);

  // ❗ 2. 이벤트 목록을 useMemo로 계산 (성능 최적화)
  const pastClubs = useMemo(() => {
    return Array.isArray(pastEvents) ? pastEvents : [];
  }, [pastEvents]);

  // ❗ 3. 지역 필터링 및 정렬 로직
  const filteredClubs = useMemo(() => {
    let filtered = pastClubs;

    // 지역 필터링
    if (selectedRegions.length > 0) {
      filtered = pastClubs.filter((club) => {
  ...
  return (
    <div className="flex w-full flex-col">
      <div className="flex-grow bg-BG-black">
        <header className="flex flex-col bg-BG-black px-[1.25rem]">
          <div className="flex w-full items-center py-[0.62rem]">
            ...
            <div className="flex w-full items-center justify-between">
              <span className="text-subtitle-20-bold text-white">과거 이벤트 관리하기</span>
            </div>
        ...
        {/* 무한스크롤 감지 요소 */}
        {pastHasMore && <div ref={observerRef} className="h-4 w-full" style={{ minHeight: '1rem' }} />}
      </div>
    </div>
  );
}
