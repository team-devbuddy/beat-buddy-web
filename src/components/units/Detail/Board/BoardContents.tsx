  ...
  return (
    <div className="bg-BG-black text-white">
      {filteredData.map((item) => (
        <div
          key={item.id}
          className={`flex flex-col space-y-1 px-4 py-4 ${item.boardType === '조각 게시판' ? 'bg-gray500' : ''}`}>
          {/* 작성자 프로필, 닉네임, 작성시간 */}
                ...
                <AnimatePresence>
                  {activeDropdown === item.id && (
                    <>
                      {/* 배경 어둡게 처리 */}
                      <motion.div
                        className="z-2 fixed inset-0 bg-black bg-opacity-50"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        onClick={() => setActiveDropdown(null)}></motion.div>
                      ...
                      <motion.div
                        ...
                        {item.isAuthor ? (
                          <>
                            ...
                            <button className="text-red500 hover:bg-red700 w-full px-4 py-2 text-left text-gray200 hover:text-main focus:text-main">
                              삭제하기
                            </button>
                          </>
                        ) : (
                          <button className="w-full px-4 py-2 text-left text-gray200 hover:text-main focus:text-main">
                            신고하기
                          </button>
                        )}
                      </motion.div>
