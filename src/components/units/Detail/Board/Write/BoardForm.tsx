  ...
  return (
    <div className="min-h-screen bg-BG-black px-4 py-6 text-white">
      ...
      <div className="mb-6">
            ...
            <div className="no-scrollbar flex items-center space-x-4 overflow-x-scroll">
              ...
              {uploadedImages.map((image, index) => (
                <div key={index} className="relative h-[9.8rem] w-[9.8rem] flex-shrink-0 rounded-xs bg-gray500">
                  <img src={image} alt={`Uploaded ${index}`} className="h-full w-full rounded-xs object-cover" />
                  {/* 삭제 버튼 */}
                  <button onClick={() => handleRemoveImage(image)} className="absolute right-1 top-1">
                    <img src="/icons/x-square-contained.svg" alt="Remove Icon" className="h-6 w-6 hover:opacity-80" />
                  </button>
                  {/* 순번 라벨 */}
        ...
        {/* 업로드 제한 안내 */}
        {uploadedImages.length >= 5 && (
          <p className="mt-2 text-start text-body3-12-medium text-main">최대 5장까지만 업로드 가능합니다.</p>
        )}
      </div>
      ...
      <div className="mb-14 flex justify-end gap-2">
        <button
          ...
          className={`rounded-xs px-[0.38rem] py-[0.13rem] text-body3-12-medium transition ${
