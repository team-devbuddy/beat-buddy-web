...
const InfoModal: React.FC<InfoModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="relative z-50">
      <div className="fixed inset-0 bg-transparent" onClick={handleBackgroundClick}></div>
      <motion.div
        ...
        {moodDescriptions.map((mood, index) => (
          <div key={index} className="flex flex-col gap-1 py-2">
            <p className="text-[0.93rem] font-bold text-main">{mood.title}</p>
            <p className="text-xs text-white">{mood.description}</p>
          </div>
        ))}
      </motion.div>
    </div>
