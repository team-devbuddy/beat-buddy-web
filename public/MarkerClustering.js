  ...
  enableClickZoom: function () {
    ...
    this._relation = naver.maps.Event.addListener(
      ...
      naver.maps.Util.bind(function (e) {
        // 클러스터 크기에 따라 적절한 줌 레벨 계산
        var clusterCount = this.getCount();
        var currentZoom = map.getZoom();
        var targetZoom;

        // 클러스터 내의 모든 마커가 화면에 보이도록 fitBounds 사용
        var clusterMembers = this.getClusterMember();
        if (clusterMembers && clusterMembers.length > 0) {
          // 클러스터 내 마커들의 bounds 계산
          var firstMarker = clusterMembers[0];
          var bounds = new naver.maps.LatLngBounds(firstMarker.getPosition(), firstMarker.getPosition());

          for (var i = 1; i < clusterMembers.length; i++) {
            bounds.extend(clusterMembers[i].getPosition());
          }

          // 패딩을 포함하여 fitBounds 실행
          ...
          // 클러스터 멤버가 없으면 기본 줌인
          targetZoom = Math.min(currentZoom + 3, 18);
          map.morph(e.coord, targetZoom);
        }
      }, this),
    );
  },
